---
title: "Kart med andel NEET i 2024 blant 15-29 åringer"
author: Lars Bauger 
format: 
  dashboard:
    orientation: columns
...
tvb_kart_neet_kontinuerlig <- leaflet(df_desc_sf_2024) |> 
  addTiles() |> 
...
tvb_kart_neet_kategorisk <- leaflet(df_desc_sf_2024) |> 
  addTiles() |> 
  setView(lng, lat, zoom = 7) |> 
  addPolygons(fillColor = ~pal_bin(neet_andel),
              stroke = TRUE, 
              fillOpacity = 0.7,
              opacity = 0.2,
              color = "white",
...
```{r}
#| title: Kart 1, Andel NEET blant 15-29 år (hele populasjonen) - Kontinuerlig fargeskala (relativ til min–maks)
tvb_kart_neet_kontinuerlig
```

```{r}
#| title: Kart 2, Andel NEET blant 15-29 år (hele populasjonen) - Trinnvis fargeskala (intervaller)
tvb_kart_neet_kategorisk
```
...
labels <- sprintf(
  ...
  <strong>%s</strong><br/>
  %g%% NEET<br/>
  Total populasjon innvandrere: %s<br/>
  Antall NEET blant innvandrere: %s<br/>
  Fylke: %s",
...
```{r}
#| title: Kart 1, Andel NEET blant 15-29 år i innvandringsbefolkningen - Kontinuerlig fargeskala (relativ til min–maks)
tvb_kart_neet_innvand_continious
```

```{r}
#| title: Kart 2, Andel NEET blant 15-29 år i innvandringsbefolkningen - Trinnvis fargeskala (intervaller)
